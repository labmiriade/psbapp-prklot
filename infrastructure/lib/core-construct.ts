...
export interface CoreConstructProps {
  ...
  /**
   * The territory parking lots csv data urls
   */
  csvDataUrls: string;
  /**
...
export class CoreConstruct extends Construct {
  constructor(scope: Construct, id: string, props: CoreConstructProps) {
    ...
    // lambda to import territory parking lots to dynamodb
    const prkLotImportLambda = new lambda.Function(this, 'PrkLotImportFn', {
      code: new lambda.AssetCode('../etl/import-prklot', {
        bundling: {
      ...
      memorySize: 512,
      description: 'Lambda per importare parcheggi territoriali',
      environment: {
        DATA_TABLE: dataTable.tableName,
        CSV_DATA_URLS: props.csvDataUrls,
      },
    });
    dataTable.grantReadWriteData(prkLotImportLambda);

    const prkLotImportErrors = prkLotImportLambda.metricErrors({
      period: cdk.Duration.minutes(1),
    });

    const importAlarm = new cloudwatch.Alarm(this, 'prkLotImportErrorsAlarm', {
      metric: prkLotImportErrors,
      threshold: 1,
      comparisonOperator: cloudwatch.ComparisonOperator.GREATER_THAN_OR_EQUAL_TO_THRESHOLD,
      evaluationPeriods: 1,
      alarmDescription: 'An error occurred during the PrkLotImport function execution',
    });
    ...
    eventRule.addTarget(new targets.LambdaFunction(prkLotImportLambda));
